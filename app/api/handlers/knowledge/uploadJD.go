...
func UploadJD(ctx context.Context, c *app.RequestContext) {
	...
	if err != nil {
		logger.ErrorCtx(ctx, "读取 JD 上传文件失败", zap.String("file_name", fileHeader.Filename), zap.Error(err))
	...
	if err != nil {
		logger.ErrorCtx(ctx, "上传 JD 文件 RPC 调用失败",
			zap.String("user_id", userID),
			zap.String("file_name", fileHeader.Filename),
			zap.Error(err))
